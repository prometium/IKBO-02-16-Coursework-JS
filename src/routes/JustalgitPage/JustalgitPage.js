...
function fetchWeather(cityName) {
  return fetch(
    'http://api.openweathermap.org/data/2.5/weather?q=' +
      cityName +
      '&units=metric&APPID=c4c5932f1c7a72395d515bba8e831179'
  )
    ...
    fetchWeather(cityInput).then(function (result) {
      if (result != undefined) {
        ...
        feelsLikeInfo.textContent =
          'Ощущается как ' + result.main.feels_like + ' градусов';
        windInfo.textContent =
          'Скорость ветра - ' + result.wind.speed + ' метров в секунду';
        humidityInfo.textContent =
          'Влажность - ' + result.main.humidity + ' процентов';
      } else alert('Ошибка - проверьте, что Вы вводите название города корректно;\nпримечание: иностранные города следует вводить на латинице.');
    });
  }
}
